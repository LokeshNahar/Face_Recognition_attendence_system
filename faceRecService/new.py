...
class RealFaceRecognizer:
    def __init__(self, model_name: str = "Facenet", threshold: float = 0.7):
        """
    ...
    def add_employee(self, employee_id: str, employee_name: str, face_images: List[np.ndarray]):
        """Add new employee with multiple face images for better recognition"""
        embeddings = []
        ...
        if embeddings:
            ...
            self.employee_embeddings[employee_id] = avg_embedding
    ...
    def recognize_face(self, face_image: np.ndarray) -> Tuple[Optional[str], Optional[str], float]:
        """
        Recognize face and return employee_id, name, and confidence score.
        Returns: (employee_id, employee_name, confidence) or (None, None, 0.0)
        """
        ...
        for employee_id, stored_embedding in self.employee_embeddings.items():
            ...
            if similarity > best_score:
                ...
                best_match_name = self.employee_metadata[employee_id]['name']
    ...
    def remove_employee(self, employee_id: str) -> bool:
        ...
        if employee_id in self.employee_embeddings:
            ...
            self.save_employee_database()
            print(f"Removed employee {employee_id}")
            return True
        return False

    def list_employees(self) -> Dict:
        """List all registered employees"""
        return self.employee_metadata

    def save_employee_database(
        self, embeddings_file: str = "employee_embeddings.pkl",
        metadata_file: str = "employee_metadata.json"
    ):
        """Save employee database to files"""
        with open(embeddings_file, "wb") as f:
            pickle.dump(self.employee_embeddings, f)
        with open(metadata_file, "w") as f:
            json.dump(self.employee_metadata, f, indent=2)

    def load_employee_database(
        self, embeddings_file: str = "employee_embeddings.pkl",
        metadata_file: str = "employee_metadata.json"
    ):
        """Load employee database from files"""
        if os.path.exists(embeddings_file):
            with open(embeddings_file, "rb") as f:
                self.employee_embeddings = pickle.load(f)
        if os.path.exists(metadata_file):
            with open(metadata_file, "r") as f:
                self.employee_metadata = json.load(f)
...
class MultiPoseRegistrationApp:
    def __init__(self):
        ...
        self.required_poses = {'front': 20, 'left': 5, 'right': 5}
        self.min_confidence = 0.6
        self.capture_delay = 0.8  # Faster for more images
